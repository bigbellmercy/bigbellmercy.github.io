...
2. `sudo fdisk -l` 명령으로 이 SD 카드의 장치 이름을 알아 내야 한다. 이에는 `Disks` 소프트웨어도 도움이 된다. 이 `fdisk` 명령을 치면, `Disk /dev/sdb`처럼 나오는 것은 그 디스크 전체이고, 그 밑의 `/dev/sdb1`처럼 나오는 것은 그 속의 한 파티션이다. 디스크 전체를 이미지 뜨려면 `/dev/sdb`를 쓰고, 어느 파티션만을 이미지 뜨려면 `/dev/sdb1`을 쓴다. 여기서는, 디스크 전체의 이미지를 뜨는 것으로 가정하여 `/dev/sdb`
4. 이 장치 이름을 가지고, `sudo umount /dev/sdb1` 명령으로 장치를 언마운트 시킨다.
