...
- 다른 대안으로서, Ubuntu 공식 사이트에서 추천하는 과정을 따라, xrdp 대신ㅇ tightvncserver를 설치하였는데, 원격 접속이 되었으나, 데스크톱 환경이 빈약하고, 심지어 terminal 소프트웨어가 실행이 되지 않고, 소프트웨어 센터에서도 제공하지 않는 등 여러가지 문제들이 나타나고, 앞으로도 계속 나타날 것으로 예상되어, 쓰기가 어려울 것으로 보인다.
- 또 다른 대안으로서, AWS에서 이러한 GUI용 원격 데스크톱 서비스를 위해, DCV라는 서비스를 제공하며, EC2에 적용하는 것이 무료 사용이고, 이를 쓰면 된다고 한다.
