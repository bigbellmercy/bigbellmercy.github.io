...
## 시간 지연 넣기
앞에서의 `node` 문장 속에 아래처럼 `launch-prefix`를 넣으면 노드 실행 전에 시간을 지연시킬 수 있다.
```
    <arg name="node_start_delay" default="2.0" />
    <node pkg="my_package" type="my_package" name="my_package" output="screen"
    launch-prefix="bash -c "sleep $(arg node_start_delay); $0 $@' ">
```
또는, 아래처럼 시간 지연을 위한 노드를 추가해서도 지연시킬 수 있다. `include`문 앞에서 시간을 지연시킬 때에 유용하다.
```
    <arg name="node_start_delay" default="2.0" />
    <node name="listener" pkg="roscpp_tutorial" type="listener"
    launch-prefix="bash -c "sleep $(arg node_start_delay); $0 $@' ">
```
